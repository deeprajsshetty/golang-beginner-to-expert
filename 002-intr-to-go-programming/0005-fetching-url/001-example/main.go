...
func main() {
	for _, url := range os.Args[1:] {
		...
		if resp.StatusCode != http.StatusOK {
			fmt.Fprintf(os.Stderr, "fetch: %s returned status code %d\n", url, resp.StatusCode)
			os.Exit(1)
		}

		_, err = io.Copy(os.Stdout, resp.Body)
		if err != nil {
			fmt.Fprintf(os.Stderr, "fetch: copying %s to stdout: %v\n", url, err)
			os.Exit(1)
		}
	}
}
...
/*
Input:
go run the-go-programming-language/002-intr-to-go-programming/0005-fetching-url/001-example/main.go https://www.rust-lang.org/
  ...
  <div class="flex flex-column flex-row-l">
    ...
    <div class="w-30-l flex-column pl0-l pr0-l pl3 pr3">
      ...
      <p class="tc f3 f2-l mt3">
        <a href="https://blog.rust-lang.org/2023/08/24/Rust-1.72.0.html" class="download-link">Version 1.72.0</a>
      </p>
    </div>
  </div>
</header>
